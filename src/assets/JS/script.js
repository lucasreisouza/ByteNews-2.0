...
async function getSession() {
  try {
    const response = await fetch(getPhpPath("sessao.php"), {
...
async function updateHeader() {
  ...
  if (session.logado) {
    ...
    if (secondaryAction) {
      ...
      secondaryAction.addEventListener(
        "click",
        () => {
    ...
    if (secondaryAction) {
      ...
      secondaryAction.href = getLoginPath().replace(
        "/login.php",
        "/cadastro.php",
      );
...
function setupFavoriteButton() {
  ...
  if (!title) return;
  const idNoticia = Number(document.body.dataset.newsId || 0);
  if (!idNoticia) return;
  ...
  button.addEventListener("click", async () => {
    ...
    formData.append(
      "acao",
      button.dataset.saved === "true" ? "remover" : "adicionar",
    );
    const response = await fetch(getPhpPath("favoritos.php"), {
      method: "POST",
      body: formData,
      credentials: "same-origin",
    });
    ...
    button.textContent = data.favorito
      ? "★ Salvo nos favoritos"
...
function setupLikeButton() {
  ...
  if (!idNoticia && !slug) return;
  const button = document.createElement("button");
  button.type = "button";
  button.className = "like-button";
  const update = (data) => {
    button.dataset.liked = data.curtido ? "true" : "false";
    button.textContent = `${data.curtido ? "♥" : "♡"} ${data.curtidas} ${data.curtidas === 1 ? "curtida" : "curtidas"}`;
  };
  fetch(
    getPhpPath(
      `curtidas.php?id_noticia=${encodeURIComponent(idNoticia)}&slug=${encodeURIComponent(slug)}`,
    ),
    { credentials: "same-origin" },
  )
  ...
  button.addEventListener("click", async () => {
    ...
    formData.append(
      "acao",
      button.dataset.liked === "true" ? "remover" : "curtir",
    );
    const response = await fetch(getPhpPath("curtidas.php"), {
      method: "POST",
      body: formData,
      credentials: "same-origin",
    });
...
function setupHomeNews() {
  ...
  if (!emphasis) return;
  fetch(getPhpPath("home-noticias.php"), { credentials: "same-origin" })
    .then((response) => response.json())
    .then((data) => {
      const makeUrl = (item) =>
        item.slug
          ? "./src/pages/news/noticia.php?slug=" + encodeURIComponent(item.slug)
          : "./src/pages/news/noticia.php?id_noticia=" +
            encodeURIComponent(item.id_noticia);
      ...
      const updateCards = (selector, items, type) => {
        ...
        items.forEach((item, index) => {
          ...
          const title = card.querySelector(
            ".news-card_title, .news-latest_content h2",
          );
          const category = card.querySelector(".news-card_category");
          const time = card.querySelector(
            ".news-card_time, .news-card_description p",
          );
          if (link) link.href = makeUrl(item);
          if (image) {
            image.src = makeImage(item);
            image.alt = item.titulo;
          }
          if (title) title.textContent = item.titulo;
          if (category) {
            category.textContent = item.categoria;
            category.className = "news-card_category ia";
          }
          if (time)
            time.textContent =
              type === "latest"
                ? new Date(
                    item.data_publicacao.replace(" ", "T"),
                  ).toLocaleDateString("pt-BR")
                : type === "hot"
                  ? "♥ " + Number(item.curtidas).toLocaleString("pt-BR")
                  : "💬 " + item.comentarios + " comentários";
        });
      };
      updateCards(
        ".news-emphasis_cards .news-card",
        data.destaques,
        "featured",
      );
      updateCards(".news-latest_cards article", data.ultimas, "latest");
      const hotItems = document.querySelectorAll(".news-rise_item");
      data.alta.forEach((item, index) => {
        const row = hotItems[index];
        if (!row) return;
        const link = row.querySelector("a");
        const title = row.querySelector(".news-rise_title-text");
        const stats = row.querySelector(".news-rise_trend");
        if (link) link.href = makeUrl(item);
        if (title) title.textContent = item.titulo;
        if (stats)
          stats.textContent =
...
function setupEditorialControls() {
  ...
  const addBlock = () => {
    ...
    select.innerHTML =
      '<option value="paragrafo">Parágrafo</option><option value="lista_ordenada">Lista ordenada</option><option value="lista_nao_ordenada">Lista normal</option><option value="titulo">Título</option>';
...
async function renderComments() {
  ...
  try {
    ...
    const response = await fetch(
      getPhpPath(
        "comentarios.php?noticia=" +
          encodeURIComponent(getNewsSlug()) +
          "&id_noticia=" +
    ...
    data.comentarios.forEach((comment) => {
      ...
      const updateLike = () => {
        const total = Number(comment.curtidas || 0);
        like.textContent = `${Number(comment.curtido) ? "♥" : "♡"} ${total} ${total === 1 ? "curtida" : "curtidas"}`;
      };
      updateLike();
      like.addEventListener("click", async () => {
        ...
        if (document.body.dataset.newsId)
          formData.append("id_noticia", document.body.dataset.newsId);
        formData.append("id_comentario", comment.id_comentario);
        formData.append(
          "acao",
          Number(comment.curtido) ? "remover_curtida" : "curtir",
        );
        const response = await fetch(getPhpPath("comentarios.php"), {
          method: "POST",
          body: formData,
          credentials: "same-origin",
        });
        if (response.ok) await renderComments();
      });
      item.append(like);
      if (
        session.logado &&
        Number(comment.id_usuario) === Number(session.id_usuario)
      ) {
        const remove = document.createElement("button");
        remove.type = "button";
        remove.className = "comment-delete-button";
        remove.textContent = "Excluir comentário";
    ...
    list.innerHTML =
      '<p class="comment-login-message">Não foi possível carregar os comentários.</p>';
  }
}
...
if (!window.byteNewsInitialized) {
  window.byteNewsInitialized = true;

...
<main class="page-shell">
	...
	<section class="editor-layout">
		<form class="editor-form" method="post" action="../php/cadastrar-noticia.php" enctype="multipart/form-data">
			...
			<div class="form-section"><span class="section-kicker">02 / Conteúdo</span><h2>Construa a matéria</h2><label>Blocos da notícia</label><div id="paragraphs-container"><div class="content-block"><select name="blocos_tipo[]" aria-label="Tipo do bloco"><option value="paragrafo">Parágrafo</option><option value="recuado">Parágrafo recuado</option><option value="titulo">Título</option></select><textarea name="blocos_conteudo[]" required rows="5" placeholder="Escreva o primeiro bloco da notícia..."></textarea></div></div><button class="add-block" type="button" id="add-block">+ Adicionar bloco</button><small>Escolha o tipo do bloco e escreva o conteúdo da matéria.</small></div>
			<div class="form-section"><span class="section-kicker">03 / Classificação</span><h2>Organize a publicação</h2><div class="form-grid"><label>Categoria<input name="categoria" required maxlength="100" placeholder="Tecnologia"></label><label>Autor<input name="autor" required maxlength="100" value="<?= htmlspecialchars($_SESSION['nome']) ?>"></label></div><label>Imagem da notícia<input type="file" name="imagem_upload" accept="image/jpeg,image/png,image/webp,image/gif" required><small>Formatos aceitos: JPG, PNG, WEBP ou GIF. Tamanho máximo: 5 MB.</small></label></div>

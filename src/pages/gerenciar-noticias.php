...
$noticias = $conexao->query('SELECT n.id_noticia, n.titulo, n.autor, n.data_publicacao, n.visualizacoes, COUNT(DISTINCT c.id_comentario) comentarios, COUNT(DISTINCT l.id_usuario) curtidas FROM noticias n LEFT JOIN comentarios c ON c.id_noticia = n.id_noticia LEFT JOIN curtidas_noticias l ON l.id_noticia = n.id_noticia GROUP BY n.id_noticia ORDER BY n.data_publicacao DESC')->fetch_all(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<main class="page-shell news-manager">
  ...
  <form method="post" action="../php/gerenciar-noticias.php" id="bulk-news-form">
    <section class="manager-list"><div class="manager-list-heading"><div><h2>Publicações</h2><span>Marque as notícias para editar ou excluir</span></div><label class="select-all-label"><input type="checkbox" id="select-all-news"> Selecionar todas</label></div>
      <?php foreach ($noticias as $noticia): ?><article class="manager-row"><label class="news-select"><input type="checkbox" name="ids[]" value="<?= (int) $noticia['id_noticia'] ?>"><span></span></label><div class="manager-title"><span class="manager-id">#<?= (int) $noticia['id_noticia'] ?></span><div><h3><?= htmlspecialchars($noticia['titulo']) ?></h3><span><?= htmlspecialchars($noticia['autor']) ?></span></div></div><div class="manager-metrics"><span>◉ <?= number_format((int) $noticia['visualizacoes'], 0, ',', '.') ?></span><span>◌ <?= (int) $noticia['comentarios'] ?></span><span>♡ <?= (int) $noticia['curtidas'] ?></span></div><div class="manager-inline-fields"><label>Autor<input name="autor[<?= (int) $noticia['id_noticia'] ?>]" value="<?= htmlspecialchars($noticia['autor']) ?>"></label><label>Data<input type="datetime-local" name="data_publicacao[<?= (int) $noticia['id_noticia'] ?>]" value="<?= date('Y-m-d\\TH:i', strtotime($noticia['data_publicacao'])) ?>"></label></div></article><?php endforeach; ?>
</section>
